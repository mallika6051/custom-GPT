...
def handle_factorial():
    entire_string = display.get()
    try:
        result = factorial(int(eval(entire_string)))
        clear_all()
        display.insert(0, result)
    except Exception:
        clear_all()
...
Button(root, text="pi", command=lambda :get_operation("*3.4")).grid(row=2,column=4)
...
Button(root, text="x!", command=lambda :handle_factorial()).grid(row=3,column=5)
